...
- 主库继续保存 `files/settings/conversations/messages`，索引产物按 generation 独立存放；`active_generation_id` 指向当前在线索引，检索只读 active generation。
- Embedding 配置区分“当前在线”和“待生效”两套状态；模型、维度或多模态能力变化时，需要先构建新的 generation，成功后再原子切换。
