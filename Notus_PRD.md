...
│   ├── knowledge.js                 # /knowledge（未选中文件时仅问答；支持手动指定参考来源）
│   ├── canvas.js                    # /canvas（点击侧边栏文件后留在当前页创作）
│   ├── settings/
...
```
GET    /api/conversations            ?kind=knowledge|canvas → Array<Conversation>
POST   /api/conversations            Body: { title?, kind?, file_id? } → Conversation
...
- M4-08 AIPanel（风格选择 + 对话 + OperationPreview）
- M4-09 新建创作入口页
