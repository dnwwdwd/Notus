# REQ-20260703-003：Notus Agent 单文件暂存修订与代码 Diff 预览
...
当前 Agent 在大规模、碎片化修改时依赖 LLM 输出 `old/new` patch，容易因漏段、格式异常或原文匹配失败导致预览无法生成。需要按 `docs/Notus Agent 暂存修订与代码 Diff 预览实现方案.md` 落地第一版单文件暂存修订机制，让 LLM 只提交修改后的完整草稿，预览 diff、应用和回滚由代码负责。
