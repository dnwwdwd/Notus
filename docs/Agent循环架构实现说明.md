...
>
> **v4.3 状态连续性修订（2026-07-30）：**
> - 前端每次启动或恢复 SSE 流均分配递增序号；已中止旧流晚到的 `interaction_request` 或 `loop_done` 不得覆盖新流的 session 状态。
> - 恢复调用结束后，`/api/agent/loop/start` 以 `runAgentLoop()` 的返回状态校验并写回数据库 session，已完成、失败或取消的恢复任务不能残留为 `waiting_confirm`。
>
> **v4.4 写作目标判断修订（2026-07-31）：**
> - 移除首轮 Loop 前基于近期文章候选和关键词的服务端写作目标预检；不再自动创建“修改 / 新建”提问卡片。
> - Agent 根据当前指令、最近对话和需要时读取的工作区状态判断新建、修改或继续讨论；只有目标、范围或操作无法定位时才调用 `ask_question_card`。
...
#### 5.2.1 跨 Session 的受管资源引用

文件承接和资源承接是两条独立链路。`agentResourceContext` 不建立通用记忆库，也不复制 Cindy 的项目上下文表；它复用已有 `agent_run_logs`、`conversation_interactions` 和 Skill/MCP 领域存储。新 session 开始时按对话读取最近的成功资源工具日志及已确认资源操作，抽取 Skill/MCP ID，随后从 `getSkill(id)`、`getServer(id)` 重新解析权威状态。无效 Skill、缺失资源和已删除对象不会进入 Prompt。

上下文预算固定为每类最多 3 个资源。若最近资源操作只留下一个同类对象，Prompt 将其标为“当前对象”，用户使用“这个”“它”“改名”等承接表达时必须先用该 ID 调用资源详情或管理工具；文章、文件和界面当前选中文档不能抢占目标。若同一最近操作 session 内出现多个同类对象，则不生成唯一绑定，要求 Agent 澄清。资源日志只保留 ID、名称、启停、传输、确认与测试摘要，Header、env、Token、Skill 正文都不进入这条链路。
...
`/start` 承担首次启动和同一 session 的暂停恢复职责：

- 首次启动：不传 `session_id`，创建新 session，开始 loop
- 继续执行：传入 `session_id`、当前 `session_token` 与可选 `interaction_id`；状态为 `waiting_confirm` 或 `running` 的 session 从 checkpoint 恢复。硬上限、提问卡片和资源确认均走这条路径。
- 每条 SSE 流只允许更新自身启动序号对应的前端状态；服务端在 `runAgentLoop()` 返回后写回相同的终态。
