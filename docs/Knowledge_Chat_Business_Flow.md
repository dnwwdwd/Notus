...
> 更新时间：2026-05-02
> 适用范围：`/knowledge` 页面当前已实现的问答、历史会话、单索引检索、条件改写/重排、回答模式、来源定位与落库链路
...
知识库页左侧文档区会按 `knowledge:file:<id>` 保存浏览位置：

- 保存 Tiptap 滚动容器的 `scrollTop` 和当前可见文本锚点。
- 普通跨页返回时优先按文本锚点恢复，找不到锚点再回退到 `scrollTop`。
