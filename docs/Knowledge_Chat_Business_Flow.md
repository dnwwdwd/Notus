...
- 输入框会随请求携带当前模型、联网搜索状态、单选搜索服务商、附件元数据和用户本轮原始输入 `user_query`；知识库页普通问答仍走 `/api/chat`，普通联网问答改走只读 Agent Loop 并只使用本轮 `user_query` 作为输入边界。联网搜索打开时注入 `web_search` 工具供模型按需重复调用；关闭时不注入联网工具，也不加载历史联网上下文。设置页联网搜索总开关实时保存，其他搜索配置仍手动保存。
