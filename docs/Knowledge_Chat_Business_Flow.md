...
- Agentic Loop 会先创建任务快照，按授权路径执行工具调用，文件改动先生成 `preview_patch_files` 预览，并在对话底部通过文件级 diff 卡片逐文件应用或回滚。
...
4. 如果生成文件级预览，在助手消息底部显示 diff 卡片。
5. 用户逐文件点击应用或回滚时调用 `/api/agent/loop/apply`；成功后只更新文件和卡片状态，不续跑 Loop。
...
- 普通用户消息仍通过 /api/chat 进入知识库问答链路，继续使用查询规划、知识库检索、证据判断、SSE 流式回答和引用来源；写作类任务直接进入 Agentic Loop，完成后通过对话底部 diff 卡片逐文件确认。
