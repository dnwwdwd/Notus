...
- 普通知识问答继续走 `/api/chat`；明确要求写入、新建笔记、整理成文章、跨文件整理或检查链接时，仍复用当前输入入口，但先展示 Agentic Loop 任务确认卡。
- Agentic Loop 任务确认后会创建任务快照，按授权路径执行工具调用，文件改动先生成 `preview_patch_files` 预览，确认后写入并可整体回滚。
...
- 写作类任务：不先插入用户消息，不请求 `/api/chat`，而是生成 `pendingAgentTask` 并显示 Agentic Loop 任务确认卡。
...
4. 助手消息只保留一条引导语，真正的问题定义放进底部 `ClarifyDrawer`。
5. 抽屉最多补问 3 个结构化条件，优先围绕对象、对比对象、时间范围和检索范围。
...
- 普通用户消息仍通过 /api/chat 进入知识库问答链路，继续使用查询规划、知识库检索、证据判断、SSE 流式回答和引用来源；写作类任务先显示任务确认卡，再进入 Agentic Loop。
