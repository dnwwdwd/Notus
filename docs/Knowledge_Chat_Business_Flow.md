...
- 右侧 AI 聊天区不保存滚动位置；仅当用户原本停留在底部时，才随流式 token、检索/工具链步骤和最终消息继续滚动。用户上滑或生成中手动滚动后，页面保持当前位置；切换模型、搜索商或其他输入框模式不触发自动定位到最新消息。
- 当用户已经离开底部时，输入框上方中间位置显示仅 icon 的回底按钮，点击后平滑滚到当前会话最底部。
- 用户消息气泡正文下方右侧提供复制和改写；复制 tooltip 为“复制”，仅复制用户消息正文。改写进入编辑态，按钮文案为“发送”，Enter 发送，Shift+Enter 换行，Esc 取消；确认后复用原用户消息附件元数据和当前联网搜索偏好重新发送。
- 改写历史用户消息时，前端先让被改写消息之后的所有消息淡出，再调用 `/api/conversations/:id/truncate` 更新目标用户消息并删除其后的消息；后续重新发送会携带 `skip_user_message_append`，让普通 `/api/chat` 或写作类 Agent Loop 复用这条已更新用户消息作为上下文起点，不再追加重复用户消息。
- AI 回复气泡正文下方左侧支持复制和重试；复制 tooltip 为“复制”，重试 tooltip 文案为“重试”，重试默认复用最近一条对应用户消息的文本、附件元数据和当前联网搜索偏好，再次触发当前知识库问答或 Loop 分流判断。
...
1. 插入用户消息；Agent 的非删除写入能力覆盖整个笔记库，当前文档路径只作为任务上下文和快照范围线索。
...
5. 用户在 DiffDialog 中逐文件或逐目录操作点击应用或回滚时调用 `/api/agent/loop/apply`，并携带当前对话 ID；成功后只更新文件树、弹窗和摘要卡状态，不续跑 Loop。新建或切换对话后，旧预览只保留查看、导出和日志复盘，不再允许继续应用或回滚。

`create_note` 新建文件不再按当前文档目录做写入拦截；后端仍校验 session token、会话状态、操作类型和删除禁用规则。
...
- 前端将 chunks / assistant_meta / token / citations / done 映射为设计稿工具过程、Notus Agent 回复正文和 `SourceCard` 垂直来源列表；点击来源仍在知识库页内部展开左侧文档并定位原文，来源列表保留选中反馈。
- 输入框会随请求携带当前模型、联网搜索状态、单选搜索服务商、附件元数据和用户本轮原始输入 `user_query`；知识库页普通问答仍走 `/api/chat`，普通联网问答改走只读 Agent Loop 并只使用本轮 `user_query` 作为输入边界。模型选择会写回全局默认 LLM 配置；知识库页和创作页输入框共用同一份浏览器本地联网搜索偏好，包含联网开关与单选搜索服务商。联网搜索打开时注入 `web_search` 工具供模型按需重复调用；关闭时不注入联网工具，也不加载历史联网上下文。设置页联网搜索总开关实时保存，其他搜索配置仍手动保存。
