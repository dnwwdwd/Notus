...
- 对“把/将 A 改为 / 换成 / 替换为 B”这类确定性改写短语，优先在 `@` 指定块、最近目标块和当前候选块里做唯一命中；唯一命中时直接按改单块处理，多命中时回退为重新确认位置
- “写一篇文章 / 写正文 / 起草文章”这类起草请求按全文编辑处理，不进入文章分析分支；只有明确出现“分析 / 评估 / 点评 / 结构 / 逻辑 / 可读性”这类分析诉求时才走文章分析
