...
- 创作主链路为”请求规划器 + 执行器”，规划器每次请求都调用 LLM（`target_resolver` 模式）判断意图、目标块和操作类型，不依赖正则打分；正则只用于无歧义词法提取（`@bN` 显式引用、`全文/整篇` 全局标记），结果作为 LLM 候选上下文
...
Sidebar = 顶部 Tab（📁/≡）+ 搜索框 + 内容区（bg:--bg-sidebar）；大纲点击后应立即显示稳定选中背景、左侧指示条和选中文字色，不只依赖滚动监听后的 active 状态。
...
**EditorArea：** H1 --text-3xl/700, H2 --text-2xl/600, H3 --text-xl/600；代码块 bg:--bg-secondary --font-mono；引用块左边框 3px --accent；GFM 表格显示外框、单元格边框、表头背景和选中单元格高亮；图片 max-width:100% shadow:--shadow-sm，并支持点击进入文档内预览；来源高亮响应 URL hash `#L24-L28`。
...
左侧文章区直接复用 WYSIWYG 编辑器和工具栏，可边看边改，也可一键收起；可见编辑区默认隐藏仅包含系统 `id` 的 frontmatter。工具栏除基础格式外，额外支持文本内容居中，并要求保存后继续可回显。问答区顶部展示当前文章提示、历史抽屉入口、新建对话 icon 按钮，以及“参考来源”区（自动匹配 / 手动指定文件）。顶部当前文档、文章预览头部和来源卡片在线播放统一使用可见文档标签：优先 `title`，其次显示去掉 `.md` 的文件名，最后才用占位文案，不暴露 `article_xxx`、裸 `fileId` 等技术标识。当前打开文档只影响优先检索与参考来源设置，不再切换聊天历史。命中 `clarify_needed` 时，知识库页不再返回单条文字追问，而是用底部 `ClarifyDrawer` 收集最多 `3` 个结构化条件，题目优先围绕对象、对比对象、时间范围和检索范围；答完后先进入回顾态，用户点击“开始检索”才真正续跑。InputBar：TextArea(flex:1) + 右下角固定工具组（模型选择器 + 发送/停止按钮）；模型选择器触发器固定在发送按钮左侧，窄宽度下单行省略显示，完整模型名通过标题提示与菜单项展示；生成中输入框只保留停止按钮，真正的等待态固定显示在 AI 回复气泡内，检索状态作为 loading 气泡内部的动态步骤切换；抽屉展开时输入栏禁用，收起后恢复普通提问；`v1` 不展示 `+` 附件入口。
...
**模型配置：** Embedding（Base URL + 模型名 + API Key + 维度只读）+ LLM 卡片列表（配置名称 + 厂商识别 + 模型名 + Base URL + 密钥状态）+ [测试连接] + [保存 / 设为默认]。系统根据 Base URL 与模型名自动识别兼容厂商，Embedding 维度在测试后自动确认；设置页和引导页 Step 1 中的 Embedding 表单在首次进入时仍以空态开始，但读取到服务端已保存配置后，会回填 Base URL 与模型名，API Key 不回显，只通过“已保存，留空不修改”占位提示反映状态。保存成功后，这两个输入框也会立即显示当前值。LLM 卡片保留默认配置说明，但去掉非默认配置的冗余说明区和“配置完整”状态徽标；LLM 预算字段仍会在后端持久化用于运行时控制，但不在卡片中直接展示。设置页需要提示：启用文档级上下文后，命中的 Markdown 正文可能进入外部对话模型请求，回答下方会展示本次读取的文件摘要。
