...
- 当主意图、来源、目标位置或写入方式不够稳定时，会在右侧 AI 面板底部抽屉中一次性补问最多 `3` 个问题；抽屉覆盖输入框，采用列表状态布局，不再把卡片和重试提示内联到消息流里；回答后自动续跑，但仍只生成待应用预览
...
左侧文章区直接复用 WYSIWYG 编辑器和工具栏，可边看边改，也可一键收起；可见编辑区默认隐藏仅包含系统 `id` 的 frontmatter。工具栏除基础格式外，额外支持文本内容居中，并要求保存后继续可回显。问答区顶部展示当前文章提示、历史抽屉入口、新建对话 icon 按钮，以及“参考来源”区（自动匹配 / 手动指定文件）。顶部当前文档、文章预览头部和来源卡片在线播放统一使用可见文档标签：优先 `title`，其次显示去掉 `.md` 的文件名，最后才用占位文案，不暴露 `article_xxx`、裸 `fileId` 等技术标识。当前打开文档只影响优先检索与参考来源设置，不再切换聊天历史。InputBar：TextArea(flex:1) + 右下角固定工具组（模型选择器 + 发送/停止按钮）；模型选择器触发器固定在发送按钮左侧，窄宽度下单行省略显示，完整模型名通过标题提示与菜单项展示；生成中输入框只保留停止按钮，真正的等待态固定显示在 AI 回复气泡内，检索状态作为 loading 气泡内部的动态步骤切换；`v1` 不展示 `+` 附件入口。
...
**状态：** 空态(✨ + 3 个 suggestion chip) / 索引未完成(⏳ + ProgressBar) / 模型未就绪(整块锁态 + 前往设置) / AI 回复中(StreamingText + 聊天气泡内 loading + 停止生成) / 需澄清(只追问，不检索) / 证据偏弱(保守回答) / 证据冲突(明确提示不同说法) / 检索无足够证据(文字提示) / API 错误(InlineError + 重试)。

### 6.6 AI 创作画布页（`/canvas`）

布局：Sidebar + CanvasArea(默认约 62%) + AIPanel。中间分栏可拖拽调整，并记住上次使用宽度。

**AIPanel：** 已保存文章时，顶部仅保留“查看历史对话 / 新建对话”两个 icon 按钮，旧会话通过 ConversationDrawer 抽屉展示；顶部当前文章标题区、对话区引用文档和来源卡片在线播放统一使用可见文档标签，不暴露内部 id 风格文案。其下是风格来源区域（自动匹配 / 手动指定） + 对话区（支持文本回复、文章分析、批量修改预览恢复，以及结构化提问卡片恢复） + InputBar（`@b2` / `@b2-b5` / “全文”快捷项，`@` 触发当前文档块 autocomplete，支持上下键切换、回车确认、Esc 关闭，中文输入法组合态不抢键，键盘移动时列表滚动条跟随选中项；模型选择器固定在右下角发送按钮左侧，触发器单行省略，菜单项展示完整模型名）。当系统无法稳定理解“把上面的内容写到文档中”“把刚才生成的内容写进去”这类编辑请求时，会在右侧面板底部抽屉中一次性补问最多 3 个问题；抽屉使用列表状态布局，只保留标题、状态、问题、选项、自定义输入和按钮，并覆盖原输入框，不再在消息流里内联展示卡片或重试块。高风险预览和澄清后续跑会额外显示“当前理解”摘要，并提供轻量纠偏入口；当同一条回复同时包含摘要和批量预览时，必须保留预览与应用按钮。事实参考由后台默认自动补充，并始终优先当前文档；系统入模时只选取相关块包、最近少量历史和请求内摘要，不再默认整篇文章全量直传；文章分析默认关闭，只保留后端开关；未保存的大纲草稿则显示“先保存再继续 AI 改写”的引导卡片，并禁用历史会话与输入栏。只要当前创作存在未保存改动，用户从侧边栏、顶部搜索或页内切换到其他文章前，都必须先经过统一的未保存确认弹窗。
...
**状态：** 加载中(Skeleton) / 模型未就绪(整页浅遮罩 + 锁态卡片) / AI 处理中(脉冲 + StreamingText + AI 气泡内柔和三点 loading) / 大纲生成中(整页蒙版 + loading 卡片，底层块内容逐步写入) / 提问卡片待回答(右侧底部列表抽屉覆盖输入框) / 提问卡片已失效(需要重新确认位置或重新发起) / 回答已记录但续跑失败(抽屉保留重试入口，点击后立即隐藏并进入生成中状态) / 高风险预览(当前理解摘要 + 轻量纠偏入口，同时保留批量预览与应用按钮)。

### 6.7 设置页（`/settings/model` 等）

布局：SettingsNav(w:200px) 替换 Sidebar + 内容区(max-width:920px mx:auto)。六项：模型配置 / 个性化 / 存储 / 日志 / 快捷键 / 关于；每项使用单独路径。
