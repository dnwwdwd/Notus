...
> v2.0 · 单一事实源 · 定义产品是什么、为谁做、解决什么问题、长什么样
...
- 来源卡片：file_title + heading_path + preview + 行号，点击跳转编辑器
...
- **从主题新建**：顶部"创作"Tab → 输入主题 → Agent 检索知识库 + 生成大纲 → 写入画布 → 逐块起草
...
```
切换到「知识库」Tab → 输入问题 → 混合检索 → 流式回答 + 来源卡片
→ 点击来源卡片 → 跳转编辑器对应行并高亮 3s
```
...
→ AI 检索风格样本 → 调用 draft_block/expand_block 等工具
...
→ 大纲骨架写入画布 → 用户调整大纲（拖拽/改标题）
→ 逐块调用 draft_block → 每块独立上下文稳定
→ 完成后保存为新 MD 文件 → 自动索引
```
...
> 本节吸收自 Notus UI Guide v2.1，是所有页面的权威定义，PRD 不重复。
...
| 5 | `/canvas` | AI 创作画布 | 是 | 左侧创作块 + 右侧对话与风格来源 |
...
左侧文章区直接复用 WYSIWYG 编辑器和工具栏，可边看边改，也可一键收起。问答区顶部支持“参考来源”设置：自动匹配或手动指定文章。InputBar：模型 Select(sm，上拉展开) + TextArea(flex:1) + 发送按钮(40x40 bg:--accent radius-full)。

**状态：** 空态(✨ + 3 个 suggestion chip) / 索引未完成(⏳ + ProgressBar) / AI 回复中(StreamingText + 停止生成) / 检索无结果(文字提示) / API 错误(InlineError + 重试)。

### 6.6 AI 创作画布页（`/canvas`）

布局：Sidebar + CanvasArea(65%) + AIPanel(35%)。

**AIPanel：** 顶部风格来源区域（自动匹配 / 手动指定，手动模式可动态添加参考文章） + 对话区（+ OperationPreview 嵌套） + InputBar（`@b2` 引用块，`@` 触发 autocomplete，模型选择框上拉展开）。
...
**无文章态：** 新建创作入口（居中 max-width:480px，主题 Input h:48px + primary "生成大纲并开始" + 最近创作列表）。

**状态：** 加载中(Skeleton) / AI 处理中(脉冲 + StreamingText) / 大纲生成中(Block 骨架依次渐入)。

### 6.7 设置页（`/settings/model` 等）

布局：SettingsNav(w:200px) 替换 Sidebar + 内容区(max-width:640px mx:auto)。四项：模型配置 / 存储 / 快捷键 / 关于；每项使用单独路径。

**模型配置：** Embedding（提供商 + 模型 + API Key + 维度只读）+ LLM（提供商 + 模型 + API Key）+ [测试连接] + [保存]。
...
**`/setup`**：三步，全屏居中 max-width:560px。步骤指示器圆点 12x12 + 连接线。
...
**基础（19 个）：** Button（4×3）、Dialog、ContextMenu、Input、TextArea、Select、SearchInput、CommandPalette（⌘K）、SearchResults、Spinner、Badge（5 变体）、Tooltip（500ms 延迟）、Toast、Toggle、Tabs、ConfirmDialog、FilePickerDialog、ProgressBar、EmptyState。

**业务（6 个）：** SourceCard（来源卡片）、OperationPreview（AI diff 预览）、CanvasBlock（画布块）、StreamingText（闪烁光标）、Skeleton（shimmer）、InlineError。
...
| 1024–1279px | Sidebar 默认折叠，overlay 展开 | AIPanel 收窄 |
| 768–1023px | Sidebar 变 drawer（☰触发） | 画布 AIPanel → 底部抽屉 |
| <768px | 同上 | 仅文件管理 + 知识库；画布/设置降级 |
