...
工作区工具层
  ...
  - `search_knowledge` 仅用于 Markdown 正文、事实材料和写作参考；文件系统结构确认必须走 `analyze_folder`，并以其返回的实时子目录和文件列表为准
...
- M2-02 FileTree 组件 + `/api/files/*` API
  ...
  - 侧边栏文件树中的右键新建文件、目录新建、目录移动、目录重命名、目录删除和文件移动直接应用并刷新文件树，不弹出 DiffDialog；右键目录新建文件落在目录内，右键文件新建文件落在同级目录内；移动目录时目标列表排除当前目录及其子目录；只有 Agent 生成的文件级或文件系统预览才进入消息摘要卡和 DiffDialog
