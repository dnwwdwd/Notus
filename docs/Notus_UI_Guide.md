...
- Sidebar 的文件树 / 大纲 tab 状态与各自滚动位置需要持久化；页面没有大纲时只临时显示文件树，不覆盖用户原本的 tab 偏好
...
左侧文章编辑区会按文件保存浏览位置；来源定位优先于历史浏览位置。
```
...
**分栏宽度：** 画布区与右侧 AI 面板之间支持拖拽调宽，并记住上次宽度。左侧创作块区会按文件保存 block 浏览位置，返回时优先按 block id 恢复。
...
- 文件页、知识库页文档区和创作页块区浏览位置按页面与文件保存；显式来源定位和 URL 行号定位优先。
