...
**对话切换区域（顶部）：** 只保留“查看历史对话 / 新建对话”两个 icon button + tooltip。创作页历史列表只按 `kind=canvas` 展示全局创作对话，不按当前文章过滤；历史列表通过 `ConversationDrawer` 从右侧滑入，每条会话右侧有删除 icon，点击后二次确认；未保存大纲时不展示历史列表，而是先引导用户保存。
...
- 侧边栏文件树右键菜单按对象类型展示：文件提供“新建文件 / 移动文件 / 重命名 / 删除”，目录提供“新建文件 / 新建目录 / 移动目录 / 重命名目录 / 删除目录”。右键目录新建文件落在目录内，右键文件新建文件落在同级目录内，新建文件按更新时间显示在同级文件列表第一个；移动目录时目标列表排除当前目录及其子目录；这些侧边栏显式文件管理动作都直接应用并刷新文件树，不弹出 `DiffDialog`；只有 Agent 生成的文件级或文件系统操作预览才进入消息摘要卡和 `DiffDialog`。
