...
- Sidebar 的文件树 / 大纲 tab 状态与各自滚动位置需要持久化；文件页和知识库页都开放 H1-H6 大纲，选中项由真实标题位置控制，切换 tab 后不得回到旧标题；页面没有大纲时只临时显示文件树，不覆盖用户原本的 tab 偏好
...
| 保存状态按钮 | 统一承载保存动作与状态。`未保存` 使用红色文字 + 红色描边；`已保存` 使用绿色文字；`保存中` 显示 spinner |
...
| 文件 icon | 📄 16x16 |
| 文件名 | font-size:--text-sm, color:--text-primary；文案统一使用可见文档标签（优先标题，其次去掉 `.md` 的文件名），禁止显示内部 id 风格名称；空间不足时可单行省略 |
...
| 保存中 | TopBar 保存按钮显示 Spinner(sm)，完成后切回已保存态 |
| 未保存 | TopBar 保存按钮显示红色文字 + 红色描边，提示当前内容尚未保存 |
...
选中文件后，左侧会额外展开文章编辑区；未选中文件时不展示编辑器。编辑区与问答区之间支持拖拽调宽，并记住上次宽度。
当前打开文档只影响优先检索与参考来源，不切换右侧聊天历史。
ChatHeader 右上角只保留两个 icon button：查看历史对话、新建对话；旧会话通过 `ConversationDrawer` 从右侧抽屉展示。
AI 页面当前沿用既有页面壳层和侧边栏行为，本轮不单独改动全局文件树、顶部栏或页面响应式断点。
左侧文章编辑区与文件页、创作页共享同一文档的最近阅读位置，并开放文章大纲；来源定位优先于历史浏览位置。AI 未就绪时只遮罩右侧问答区，不影响文章和大纲查看。
```
...
**AI 回复：** 左对齐, 无边框, 外层上下间距 16px, 头部 ✨+Notus label(--text-sm --text-secondary)，头部与正文间距 8px，普通正文不使用带背景和 padding 的正文气泡，直接渲染 markdown + StreamingText 组件；补充说明、文档摘要和 SourceCard 列表沿正文自然向下排列；默认自然中文对话，不固定套用“结论 / 整理 / 证据”标题。首 token 到来前固定渲染 AI 气泡占位，并在气泡内使用柔和三点等待态；知识库检索状态作为 AI 回复内部的动态步骤切换，例如“分析问题 / 检索笔记 / 找到证据 / 组织答案”，不单独占用固定状态条。
...
**对话切换区域（顶部）：** 只保留“查看历史对话 / 新建对话”两个 icon button + tooltip。仅展示绑定到当前文章的创作对话；历史列表通过 `ConversationDrawer` 从右侧滑入，每条会话右侧有删除 icon，点击后二次确认；未保存大纲时不展示历史列表，而是先引导用户保存。
...
- 通过 `/canvas?fileId=` 从文件页或知识库页打开已有文章时，文章尚未加载完成前展示“正在打开文档…”工作区骨架，不得短暂显示新建创作入口。
...
| 提问待确认 | 右侧 AI 面板底部列表抽屉弹出并覆盖输入栏 |
| 提问回顾态 | 所有答案收口到回顾列表，允许返回上一题修改，再点击“开始生成预览” |
| 续跑失败 | 抽屉保留“重试生成预览”，不在消息流里额外插入重试块 |
...
bg:--bg-elevated, border:1px --border-primary, radius:--radius-md, padding:--space-3。hover border-color:--accent shadow:--shadow-sm。文件名 --text-sm/500，统一使用可见文档标签，不显示内部 id 风格文案；heading path --text-xs --text-tertiary(" › "分隔), 摘要 --text-xs --text-tertiary truncate。多卡片垂直 gap:--space-2。
...
**键盘：** 打开后输入框自动聚焦；↑↓移动高亮, Enter 执行, Esc 关闭, 无匹配 "没有找到结果" 居中
...
- 侧边栏支持全局折叠/展开状态、文件树 / 大纲 tab 状态和各 tab 滚动位置持久化。
- 文件页、知识库页文档区和创作页块区保留页面级位置，并共享同一文档的最近阅读位置；普通滚动停止后再保存，切页时同步写入；显式来源定位和 URL 行号定位优先。
- 知识库页和创作页分栏需限制左右面板下限，保证侧边栏、编辑区和聊天区同时可见；AI 输入框右下角工具组不能换行散开；`@` 块候选用键盘上下移动时，候选列表滚动条必须跟随当前高亮项。
