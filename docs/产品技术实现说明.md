...
`session_created` 还携带已入库的 `user_message_id`；前端以该真实 ID 渲染本轮用户消息，避免后续改写引用临时 ID。`useAgentLoopController` 在处理 `session_created`、`interaction_request` 和 `loop_done` 等连续 SSE 事件时，先同步更新 `sessionRef` 再提交 React state，保证提交提问卡片时使用当前 session token；每次启动或恢复 Loop 赋予单调递增序号，取消旧流后到达的事件不得改写新流的 session 状态。提问卡未提交答案保存于 `FileAgentWorkspace` 内存，`ResizableLayout` 仅折叠左右面板而不卸载工作区；任务运行、等待回答或提交回答时，父页锁定 AI 面板。恢复调用结束后，`/api/agent/loop/start` 将数据库 session 状态与 `runAgentLoop()` 返回的终态校验并写回，避免已完成任务残留 `waiting_confirm`。改写用户消息时，前端必须先调用 `/api/conversations/:id/truncate` 并成功返回；点击发送后，`UserMessageRow` 立即以新 prompt 渲染用户消息气泡并关闭 textarea，失败才恢复编辑态。新 Agent session 立即启动，后续消息的淡出定时器不阻塞请求。服务端更新锚点消息的正文、`user_query` 与可识别的 `agent_goal`，删除其后的全部消息，取消后续未完成 session、交互和 operation set；新的 Agent session 只从截断后的数据库历史构建上下文。
...
16. 提问卡部分回答在收起编辑器、切换文件或切换面板组合后仍保留；任务运行、等待回答或提交回答时，AI 面板不允许收起。提交有效回答后，真实 LLM 会话完成时 session 不得停在 `waiting_confirm`。
