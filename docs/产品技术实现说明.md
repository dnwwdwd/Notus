...
- `GlobalAgentFiles` 使用 540px 容器、156px 单项基准宽度的分段 Tab，编辑内容与服务端内容比较 dirty 状态，未修改时禁用取消和保存。API、Hash 冲突和历史结构不变。
