...
`ToolChain` 只消费队列确认、持久化 `agent_run_events` 或恢复时间线生成的步骤；`loading` 不得生成前端虚构的 `prepare` 节点。节点仅在存在详情、工具参数、结果或恢复操作时可展开，尾部运行、暂停、失败和取消节点自动展开。`stop_agent` 由时间线头部回调 `onStop`，输入区不渲染停止图标。`AgentWorkspace` 与 `AgentInput` 共享 `min(860px, calc(100% - 32px))` 内容列；输入默认三行，CSS 在 560px 以下把工具栏收敛为单列并让次要工具横向滚动。

`MessageList` 只在 `activeSteps` 或已保留的流式草稿存在时创建 `AgentTaskTimeline`。时间线保持独立任务节点，不复用 `AssistantMessageRow` 及其操作区；节点自身在有活动时显示 N 头像与“Notus Agent”名称，既明确执行主体，也不会在网络连接尚未产生真实事件时生成空白助手消息。恢复时间线将 `created / queued_resume` 映射为“任务已暂停，执行记录已保留”和“继续任务”；`waiting_retry / waiting_model_recovery` 映射为可恢复错误节点。`cancelled` 不生成恢复操作。
...
`session_created` 携带 `protocol_version: 2`、真实 `user_message_id`、session 标识和短期控制票据；前端以真实消息 ID 渲染本轮用户消息。新链路只输出 `progress / artifact / final` 三类用户语义事件：`progress` 只服务运行态，`artifact` 承载 interaction、operation set 和限制状态，`final` 只出现一次并携带累计 usage。`useAgentLoopController` 同时保留旧事件解析以兼容 0.1.13 之前的流，每次启动或恢复都分配单调递增序号，旧流迟到事件不得覆盖新流状态。提问卡未提交答案保存于 `FileAgentWorkspace` 内存，刷新后通过会话详情恢复 pending interaction 或 queued resume job；回答和唯一 resume job 在同一事务内提交，前端再以 `resume_job_id + resume_ticket` 请求 `/api/agent/sessions/:id/resume-interaction`。改写消息仍先截断服务端历史，再从截断后的上下文创建新 session。

`mapConversationMessages()` 将数据库消息的 `created_at` 映射为前端 `createdAt`。实时用户消息使用 `session_created.created_at`，实时 AI 回复在最终消息写入时记录当前创建时间；历史重新加载后以数据库 `created_at` 覆盖临时值。`utils/messageTimestamps.js` 将 SQLite 无时区 UTC 字符串按 UTC 解析，再转换为本地时间，避免时区偏移。当天显示“HH:mm”；昨天显示“昨天 HH:mm”；前天到最近 7 个日历日内显示“周几 HH:mm”；更早显示“YYYY-MM-DD HH:mm”，统一不显示秒。
...
- `POST /api/interactions/:id/respond` 在同一事务内保存回答并创建或返回唯一 resume job；同一 interaction 不会产生第二个续跑任务。会话详情签发短期 respond、resume、read、cancel、rollback、extend 和 operation ticket。
...
`agent_task_queue` 与 Agent session 一对一保存任务输入引用、LLM 配置 ID、确认模式、队列顺序、运行状态与最终消息 ID。`/api/agent/loop/start` 只落库并返回 202；运行时 Worker 领取任务，同一 conversation 的前序非终态任务阻塞后续任务。`agent_run_events` 先持久化再通过 `/api/agent/sessions/:id/events?after=` SSE 回补和订阅。进程启动将 orphaned running 任务重新入队；waiting retry/model recovery 保持暂停，取消任务不再复活。
