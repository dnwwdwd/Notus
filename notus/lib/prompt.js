  ...
  const modeInstructions = mode === 'intent_arbiter'
    ...
    : mode === 'risk_validator'
      ...
      : [
        ...
        '关键规则：没有明确修改动词时，不要因为上下文里有文章块就默认 edit。',
        '只有用户明确引用块（如 @b2、第 3 段、全文）、承接上一轮明确编辑建议（如“按刚才建议改”），或使用写入、替换、生成到文档、应用到正文等表达时，才可以在缺少修改动词的情况下偏向 edit。',
        '如果用户只是询问“怎么样 / 是否清楚 / 有什么建议 / 怎么看”，且没有要求直接写入正文，优先输出 primary_intent=text。',
        '意图示例：',
        '"将 Bun 换为 Node.js" → edit + rewrite（替换句式）',
        '"把标题改成更简洁的" → edit + rewrite（含"改"）',
        '"按刚才建议改 @b2" → edit + polish（承接上一轮明确编辑建议）',
        '"这段逻辑清楚吗" → text（提问）',
  ...
  return [
    {
      ...
      content: [
        ...
        'replace.new 必须是目标块修改后的完整全文，不能只输出修改片段。',
        '局部修改时，只改用户要求修改的部分，目标块中未修改的文字、换行、列表顺序和标点必须逐字保留。',
        'replace.old 必须逐字复制目标块原文；如果目标块很长，也不能省略、摘要或截断 old/new。',
        '目标块必须按完整正文处理；非目标块只用于理解上下文，不得出现在 replace/delete 操作里。',
        '示例：目标块原文是“第一句。第二句。第三句。”，用户只要求修改第二句时，new 也必须输出“第一句。修改后的第二句。第三句。”，不能只输出“修改后的第二句。”',
        '如果无法保证 old/new 是完整目标块全文，返回 {"summary":"无法保证完整修改","operations":[]}。',
        '不要修改未提供的块，不要返回 move 之类系统不支持的操作。',
        '如果无法生成操作，返回 {"summary":"无法生成操作","operations":[]}，不要输出任何非 JSON 内容。',
