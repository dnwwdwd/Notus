...
const retriedConversation = ensureConversation({kind:'agent',title:'重试后重启'});
const originalUser = appendConversationMessage({conversationId:retriedConversation.id,role:'user',content:'原始请求'});
const originalRun = createSession({goal:'原始请求',conversationId:retriedConversation.id});
createTask({sessionId:originalRun.sessionId,conversationId:retriedConversation.id,userMessageId:originalUser});
updateSessionStatus(originalRun.sessionId,'completed');
require('../lib/agentSession').recordRunEvent({sessionId:originalRun.sessionId,event:{type:'final',status:'completed',text:'即将被替换的旧答案'}});
require('../lib/agentTaskQueue').persistTaskFinalMessage(originalRun.sessionId,{content:'即将被替换的旧答案',status:'completed'});
rewriteConversationFromMessage({conversationId:retriedConversation.id,messageId:originalUser,content:'原始请求'});
recoverOrphanedTasks();
assert.strictEqual(getDb().prepare("SELECT count(*) n FROM messages WHERE conversation_id=? AND role='assistant'").get(retriedConversation.id).n,0,'重启不得复活用户重试删除的旧答案');
assert.strictEqual(require('../lib/agentTaskQueue').persistTaskFinalMessage(originalRun.sessionId,{content:'迟到的旧答案',status:'completed'}),null);
assert.strictEqual(appendConversationMessage({conversationId:retriedConversation.id,role:'assistant',content:'迟到的澄清卡',meta:{session_id:originalRun.sessionId,status:'waiting_interaction'}}),null);
assert.strictEqual(appendConversationMessage({conversationId:retriedConversation.id,role:'assistant',content:'迟到的能力限制',meta:{session_id:originalRun.sessionId,status:'failed'}}),null);
const newRun = createSession({goal:'原始请求',conversationId:retriedConversation.id});
createTask({sessionId:newRun.sessionId,conversationId:retriedConversation.id,userMessageId:originalUser});
updateSessionStatus(newRun.sessionId,'completed');
require('../lib/agentSession').recordRunEvent({sessionId:newRun.sessionId,event:{type:'final',status:'completed',text:'新答案'}});
recoverOrphanedTasks();
assert.deepStrictEqual(getDb().prepare("SELECT content FROM messages WHERE conversation_id=? AND role='assistant'").all(retriedConversation.id).map(r=>r.content),['新答案'],'新任务漏提交的答案仍须恢复');
