...
assert.ok(globalStyles.includes('.notus-file-agent-workspace__interaction-drawer .notus-agent-question-card__review-row {') && globalStyles.includes('min-height: 66px;'), '最终确认总览必须使用紧凑复核行，而不是整屏问题卡');
...
assert.ok(interactionRespondRoute.includes('requestCancellation(agentSessionId)'), '取消提问必须请求终止原 Agent session');
assert.ok(interactionRespondRoute.includes('cancelTask(agentSessionId)'), '取消提问必须取消原 Agent 队列任务');
assert.ok(interactionRespondRoute.includes('should_continue: false'), '取消提问后不得继续原 Agent session');
assert.ok(!interactionRespondRoute.includes('createOrGetResumeJob({ sessionId: agentSessionId, interactionId: cancelled.id, ownerId })'), '取消提问不得创建原 session 的恢复任务');
assert.ok(controller.includes('const markAgentSessionStatus = useCallback'), '取消提问后必须立即更新当前 Agent session 的本地终态');
assert.ok(controller.includes('onOperationConfirmation'), '高风险全文修订等待确认时必须通知工作区打开 Diff');
assert.ok(controller.includes("content: event.text || '已生成文件修改预览，请检查后决定是否应用。'"), '文件确认事件必须生成携带 operation set 的助手消息');
assert.ok(controller.includes('onOperationConfirmation?.(operationSet);'), '文件确认事件必须把完整 operation set 交给 Diff 入口');
...
assert.ok(fileWorkspace.includes('function buildOperationConfirmationMessage'), '历史等待文件确认必须重建可打开的 Diff 消息');
assert.ok(fileWorkspace.includes('waiting_operation_confirmation'), '刷新恢复时必须识别待确认文件修改会话');
assert.ok(fileWorkspace.includes('operationSetToOpen={operationSetToOpen}'), '文件工作区必须把待确认修改交给 Diff 弹窗');
assert.ok(fileWorkspace.includes('sessionChangeSet && isTerminalAgentSessionStatus(sessionChangeSet.session_status)'), '等待确认的任务变更集不得遮蔽单批 Diff 卡');
assert.ok(workspace.includes('operationSetToOpen = null'), 'Agent 面板必须接收待确认的 operation set');
assert.ok(workspace.includes('setDetailOperationSet(operationSetToOpen);'), '高风险全文修订必须自动打开 Diff 弹窗');
...
assert.ok(toolChainSource.includes('const hasToolActivity'), '工具链必须区分真实工具活动和纯任务状态');
assert.ok(toolChainSource.includes('className="notus-agent-toolchain__summary-static"'), '没有真实工具时只能显示不可折叠的状态行');
...
assert.ok(!toolChainSource.includes('notus-agent-toolchain__stop'), '工具链不得显示停止按钮；中断入口仅保留在 AI 输入框');
...
assert.ok(globalStyles.includes('max-width: min(var(--notus-model-name-max-width, 96px), 96px);'), '窄面板模型选择须继续缩略显示');
assert.ok(globalStyles.includes('max-width: min(var(--notus-model-name-max-width, 64px), 64px);'), '最窄面板模型选择须继续缩略显示');
assert.ok(workspace.includes('const [modelNameMaxWidth, setModelNameMaxWidth] = useState(150);'), '模型名称需要保存按实际可用宽度计算出的上限');
assert.ok(workspace.includes('const modelNameWidth = modelName.scrollWidth;'), '模型名称宽度必须按实际渲染文字测量');
assert.ok(workspace.includes('const availableWidth = Math.max(64, Math.floor(footerWidth - toolsWidth - primaryActionWidth - 24));'), '模型名称不得挤占输入区工具和发送按钮');
assert.ok(workspace.includes('new ResizeObserver(updateModelNameWidth)'), '面板拖动或窗口缩放后必须重新计算模型名称宽度');
assert.ok(workspace.includes("'--notus-model-name-max-width': `${modelNameMaxWidth}px`"), '计算结果必须传入模型选择器样式');
assert.ok(workspace.includes('disabled={!selectedConfig || !modelNameTruncated}'), '完整展示的模型名称不应重复弹出 Tooltip');
