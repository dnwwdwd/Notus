...
assert.ok(workspace.includes('const renderedSteps = useMemo(() => visibleSteps.filter((step) => step?.errorType !== \'agent\')'), 'Agent 错误步骤不能进入带分隔线的普通工具步骤列表');
...
assert.ok(workspace.includes("if (!Number.isFinite(duration) || duration < 1000) return '不到 1 秒';"), '短任务的工具链时长不得显示为 0 秒');
