...
assert.ok(fileWorkspace.includes('restoredConversationRef.current = true;\n    conversationLoadSequenceRef.current += 1;'), '用户新建对话后不得被初始化中的旧对话恢复请求写回');
...
assert.ok(workspace.includes('任务已提交，正在准备执行'), '任务刚提交时必须在工具记录位置给出真实的等待反馈');
...
assert.ok(globalStyles.includes('grid-template-columns: auto minmax(0, 1fr);'), '窄面板工具区须按附件和任务控制分列');
