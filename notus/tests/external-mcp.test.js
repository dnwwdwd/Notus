...
async function run() {
  ...
  try {
    ...
    assert.ok(iconSource.includes('SKILL_ICON_PATHS.map') && iconSource.includes('m12 2.5 8 4.5v10L12 21.5 4 17V7z'), 'Skill 图标应使用等距立方体轮廓');
    assert.ok(iconSource.includes('m4 12 8 4.5 8-4.5'), 'Skill 图标应包含等高两层方块的分隔线');
