...
const shanghai = { now, timeZone: 'Asia/Shanghai' };

assert.strictEqual(
  formatMessageTimestamp('2026-07-23 05:15:00', shanghai),
  '13:15',
  '当天消息只显示本地时间。'
);
assert.strictEqual(
  formatMessageTimestamp('2026-07-22T06:29:59.000Z', shanghai),
  '昨天 14:29',
  '昨天消息显示相对日期和时间，不显示星期。'
);
assert.strictEqual(
  formatMessageTimestamp('2026-07-21T06:29:59.000Z', shanghai),
  '周二 14:29',
  '前天到最近 7 个日历日内显示星期和时间。'
);
assert.strictEqual(
  formatMessageTimestamp('2026-07-16T06:29:59.000Z', shanghai),
  '2026-07-16 14:29',
  '超过 7×24 小时的消息应显示完整日期与分钟。'
);
assert.strictEqual(
  formatMessageTimestamp('2026-07-16T06:30:00.000Z', shanghai),
  '2026-07-16 14:30',
  '达到 7×24 小时边界的消息应显示完整日期与时间。'
);
assert.strictEqual(formatMessageTimestamp('invalid', { now }), '');
assert.strictEqual(
  formatMessageTimestamp('2026-07-23 05:15:00', { now, timeZone: 'UTC' }),
  '05:15',
  '显式时区必须覆盖运行进程时区。'
);
